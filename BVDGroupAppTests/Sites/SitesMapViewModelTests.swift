//
//  SitesMapViewModelTests.swift
//  BVDGroupAppTests
//
//  Created by khushbu on 20/06/24.
//

import XCTest
@testable import BVDGroupApp

// Test class for SitesViewModel
final class SitesViewModelTests: XCTestCase {
    
    var model: SitesViewModel!
    var mockService: MockFuelSiteService!
    
    override func setUpWithError() throws {
        model = SitesViewModel()
    }

    override func tearDownWithError() throws {
        model = nil
        mockService = nil
        UserDefaults.standard.removeObject(forKey: "userDetails")
    }
    
    func testFetchSitesData_Success() async throws {
        model = SitesViewModel(client: MockFuelSiteService(mockType: .success))
        _ = await model.fetchSitesData()
        XCTAssert(model.arrFuelSites.count > 0)
    }
    
    func testFetchSitesData_Failure() async throws {
        model = SitesViewModel(client: MockFuelSiteService(mockType: .failure))
        _ = await model.fetchSitesData()
        XCTAssert(model.arrFuelSites.count <= 0)
    }

    
    func testAuthTokenRetrieval() throws {
        let user = User(token_type: "Bearer", expires_in:86400, access_token: "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..mUurs3GjfWURWOyOo5z4iri3QVT2Q6riIGfjC2o8R4_yholSWBD7uvpc-ejVN-VQOuiVBJVw7LrDLn9sr5jpkA", refresh_token: "", user_id: 10825, first_name: "Test New", last_name: "app", email: "testapp1@gmail.com", phone: "+11111111111", term_accepted: 0, twofactor_accepted: 0)
        let encoder = JSONEncoder()
        if let encoded = try? encoder.encode(user) {
           UserDefaults.standard.setValue(encoded, forKey: "userDetails")
        }
       XCTAssertNotNil( model.authToken)
    }
    
    func testAuthTokenRetrieval_NoToken()  {
        UserDefaults.standard.removeObject(forKey: "userDetails")
        XCTAssertNil(model.authToken)
    }
}
